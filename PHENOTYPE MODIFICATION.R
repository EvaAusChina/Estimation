...
csv <- read.csv("F:/phenotype estimation/phenotypes.both_sexes.csv", sep = ";", stringsAsFactors = FALSE)
...
phenofile <-readRDS("F:/phenotype estimation/sojo/20190901_R2.rds")
...
##<<<<<<<<<<<<<<<<<<
#####################################################################################################################
###delete the known components
select <- !(xinformat$phenotype %in% res$selected.markers)
xxcor1 <-  xxcor[select, select]

xinformat1 <- xinformat[select,]
yxcor1 <- yxcor[select]
df.sojo1 <- data.frame(var.x = xinformat1$var, cor.x.y = yxcor1)
res1 <- sojo.phenotype(sum.stat.discovery = df.sojo1, cor.X = xxcor1, v.y = yinformat$var, nvar = 20)
res.insample1 <- sojo.phenotype(sum.stat.discovery = df.sojo1, sum.stat.validation = df.sojo1, 
                                  cor.X = xxcor1, v.y = yinformat$var, v.y.validation = yinformat$var, nvar = 20)
res1$selected.markers <- c(res1$selected.markers, "other")

lines(res.insample1$R2)
reslist <- cbind(res1$selected.markers, res1$lambda.v, res.insample1$R2)
colnames(reslist) <- c("variable", "lambda", "r2")

matplot(log(res1$lambda.v), t(as.matrix(res1$beta.mat)), lty = 1, type = "l", xlab = expression(paste(log, " ",lambda)), 
        ylab = "Coefficients", main = "Summary-level LASSO")
####################################################################################################################
##>>>>>>>>>>>>>>>>>>>
...
##<<<<<<<<<<<<<<<<<<<<
####################################################################################################################
### new criteria for lambda & R
#res2 <- sojo.phenotype(sum.stat.discovery = df.sojo1, cor.X = xxcor1, v.y = yinformat$var, nvar = 20)
# names <- res2$selected.markers 
# newlambda <- res1$lambda.v
#res.insample <- sojo.phenotype(sum.stat.discovery = df.sojo1, sum.stat.validation = df.sojo1, 
#                               cor.X = xxcor1, v.y = yinformat$var, v.y.validation = yinformat$var, nvar = 20)
#newR2 <- res.insample$R2
# for(i in 2:21){
  ###select1, xinformat1 and yxcor1 are data from the first selection
  # select1 <- !(xinformat1$phenotype %in% names[1:i-1])
  # xxcor2 <-  xxcor1[select1, select1]
  # xinformat2 <- xinformat1[select1,]
  # yxcor2 <- yxcor1[select1]
  
  # df.sojo2 <- data.frame(var.x = xinformat2$var, cor.x.y = yxcor2)
  #update lambda
  # res2 <- sojo.phenotype(sum.stat.discovery = df.sojo2, cor.X = xxcor2, v.y = yinformat$var, nvar = 21-i)
  # newlambda[i] <- res2$lambda.v[1]
  #update r2
#  res.insample2 <- sojo.phenotype(sum.stat.discovery = df.sojo2, sum.stat.validation = df.sojo2, 
#                                  cor.X = xxcor2, v.y = yinformat$var, v.y.validation = yinformat$var, nvar = 21-i)
#  newR2[i] <- res.insample2$R2[2]
  # i <- i+1
# }


## plot LASSO result for new lambda
# matplot(log(newlambda), t(as.matrix(res$beta.mat)), lty = 1, type = "l", xlab = expression(paste(log, " ",lambda)), 
        # ylab = "Coefficients", main = "Summary-level LASSO")
# res2$selected.markers <- c(res2$selected.markers, "other")

# reslist <- cbind(res2$selected.markers,newlambda, res.insample1$R2)
# colnames(reslist) <- c("variable", "lambda", "r2")

# lambda_compare <- cbind(res2$lambda.v, newlambda)
####################################################################################################################
##>>>>>>>>>>>>>>>>>>
